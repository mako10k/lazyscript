...
### `#pragma` の実装
環境変数オプションやコマンドラインオプションを #pragma で指定できるようにしたらどうでしょうかね？
デバッグなどがしやすいようなきがする。

### [緊急] 文法の整理
（Sprint 移動済み: 91_sprint_backlog.md の 0. に統合。後方互換はこの段階では考慮しない方針。）
AdHocの積み重ねで本来の意図と変わってきてる部分がある。
一度、テストを中断し、こちらを整備してから再度
テストコードを見直してほしい。
...
`~~bind expr1 (\pat -> expr2);`

`<-` の左腕は パターン (現在はなんか名前だけや ~名前 のような実装になってそう)

また、 Haskell の Do記法との類似から 最後は `~~return` は暗黙で入らない形だと思う。

ここ辺り、しっかりした議論が必要かと思います。

また、名前空間の第一引数は もう シンボル固定でそれ以外はエラーとしましょう。
表記がぶれてややこしくなってきてる。