...
int main(int argc, char** argv) {
  int strict = 0; int from_ls = 0; int debug = 0;
  ...
  int opt; const char* from_ls_path = NULL;
  while ((opt = getopt_long(argc, argv, "sdh", longopts, NULL)) != -1) {
    switch (opt) {
      case 's': strict = 1; break;
      case 1000: from_ls = 1; from_ls_path = optarg; break;
      case 'd': debug = 1; (void)debug; break;
      case 'h': printf("Usage: %s [--from-ls FILE] [--strict-effects]\n", argv[0]); return 0;
      default: break;
    }
  }
