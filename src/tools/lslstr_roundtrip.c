...
static int cmp_files(const char* a, const char* b){
  FILE* fa=fopen(a,"rb"); FILE* fb=fopen(b,"rb"); if(!fa||!fb){ if(fa)fclose(fa); if(fb)fclose(fb); return -1; }
  int eq=1; unsigned char ba[4096], bb[4096];
  for(;;){ size_t ra=fread(ba,1,sizeof ba,fa); size_t rb=fread(bb,1,sizeof bb,fb); if(ra!=rb){eq=0;break;} if(ra==0)break; if(memcmp(ba,bb,ra)!=0){eq=0;break;} }
  fclose(fa); fclose(fb); return eq?0:1;
}

static int dump_lstr_to_mem(const char* path, char** out_str, size_t* out_len){
  if(!out_str||!out_len) return -1;
...
int main(int argc, char** argv){
  ...
  int diff = cmp_files(in, out);
  if (diff==0){
    printf("roundtrip: equal\n");
    return 0;
  }
  // Fall back to semantic equality via LSTR dump comparison
  char *s1=NULL,*s2=NULL; size_t n1=0,n2=0;
  if (dump_lstr_to_mem(in, &s1, &n1)==0 && dump_lstr_to_mem(out, &s2, &n2)==0){
