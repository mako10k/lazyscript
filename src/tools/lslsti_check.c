...
int main(int argc, char** argv) {
  ...
  {
    typedef struct { uint32_t magic; uint16_t vmaj, vmin; uint16_t sect_cnt; uint16_t align_log2; uint32_t flags; } test_lsti_hdr_t;
    typedef struct { uint32_t kind; uint32_t reserved; uint64_t file_off; uint64_t size; } test_lsti_sect_t;
  ...
  fprintf(stderr, "dump: sect_cnt=%u align=%u size=%ld\n", (unsigned)hdr->sect_cnt, (unsigned)hdr->align_log2, (long)img.size);
  ...
  fprintf(stderr, "  sect[%u]: kind=%u off=%llu sz=%llu\n", (unsigned)i, (unsigned)sect[i].kind,
      ...
      if (sect[i].kind == (uint32_t)LSTI_SECT_STRING_BLOB || sect[i].kind == (uint32_t)LSTI_SECT_SYMBOL_BLOB) {
        uint32_t ib = 0; memcpy(&ib, base + (lssize_t)sect[i].file_off, sizeof(uint32_t));
  fprintf(stderr, "    blob index_bytes=%u\n", (unsigned)ib);
        if (sect[i].size >= 4 + ib) {
          uint32_t cnt=0; memcpy(&cnt, base + (lssize_t)sect[i].file_off + 4, sizeof(uint32_t));
  ...
  fprintf(stderr, "    patt index_bytes=%u\n", (unsigned)ib);
        if (sect[i].size >= 4 + ib) {
          uint32_t pc=0; memcpy(&pc, base + (lssize_t)sect[i].file_off + 4, sizeof(uint32_t));
          fprintf(stderr, "    patt count=%u\n", (unsigned)pc);
        }
      }
      if (sect[i].kind == (uint32_t)LSTI_SECT_THUNK_TAB) {
        uint32_t n=0; memcpy(&n, base + (lssize_t)sect[i].file_off, sizeof(uint32_t));
  ...
  // 1) Corrupt ROOTS count to exceed ncnt
  {
    ...
    const test_lsti_sect_t* thunk_tab = NULL; const test_lsti_sect_t* roots = NULL;
    for (uint16_t i = 0; i < hdr->sect_cnt; ++i) {
      if (sect[i].kind == (uint32_t)LSTI_SECT_THUNK_TAB) thunk_tab = &sect[i];
      if (sect[i].kind == (uint32_t)LSTI_SECT_ROOTS) roots = &sect[i];
    }
    if (thunk_tab && roots) {
      ...
      uint32_t badcnt = ncnt + 1; memcpy(rp, &badcnt, sizeof(uint32_t));
  ...
  // 2) If STRING_BLOB present, set STR node's len to exceed payload
  {
    ...
    const test_lsti_hdr_t* hdr = (const test_lsti_hdr_t*)dup;
    const test_lsti_sect_t* sect = (const test_lsti_sect_t*)(dup + sizeof(test_lsti_hdr_t));
    const test_lsti_sect_t* thunk_tab = NULL; const test_lsti_sect_t* sblob = NULL;
    for (uint16_t i = 0; i < hdr->sect_cnt; ++i) {
      if (sect[i].kind == (uint32_t)LSTI_SECT_THUNK_TAB) thunk_tab = &sect[i];
      if (sect[i].kind == (uint32_t)LSTI_SECT_STRING_BLOB) sblob = &sect[i];
    }
    if (thunk_tab && sblob) {
      uint32_t ib = 0; memcpy(&ib, dup + (lssize_t)sblob->file_off, sizeof(uint32_t));
