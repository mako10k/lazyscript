...
static lsthunk_t* cb_println(lssize_t argc, lsthunk_t* const* args, void* data) {
  ...
  if (!r)
    return NULL; // error or effect guard violation propagated
...
static ls_builtin_module_t CORE_MOD = {
  1,   "core", PACKAGE_VERSION, CORE_ENTRIES, (int)(sizeof(CORE_ENTRIES) / sizeof(CORE_ENTRIES[0])),
  NULL
};

__attribute__((visibility("default"))) ls_builtin_module_t* lazyscript_builtin_init(void) {
  return &CORE_MOD;
}
