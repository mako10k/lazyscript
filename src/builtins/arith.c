...
lsthunk_t* lsbuiltin_lt(lssize_t argc, lsthunk_t* const* args, void* data) {
  (void)data; (void)argc;
  lsthunk_t* lhs = ls_eval_arg(args[0], "lt: arg1"); if (lsthunk_is_err(lhs)) return lhs;
  lsthunk_t* rhs = ls_eval_arg(args[1], "lt: arg2"); if (lsthunk_is_err(rhs)) return rhs;
  if (!lhs || !rhs) return ls_make_err("lt: arg eval");
  if (lsthunk_get_type(lhs) != LSTTYPE_INT || lsthunk_get_type(rhs) != LSTTYPE_INT)
