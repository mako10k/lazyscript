...
lsthunk_t* lsbuiltin_prelude_require(lssize_t argc, lsthunk_t* const* args, void* data) {
  ...
  if (!tenv)
    return ls_make_err("require: no env");
  lsthunk_t* pathv = ls_eval_arg(args[0], "require: path");
  if (lsthunk_is_err(pathv))
    return pathv;
  ...
  lsstr_print_bare(fp, LSPREC_LOWEST, 0, lsthunk_get_str(pathv));
  fclose(fp);
  const char* path = buf;
  ...
  if (!prog)
    return ls_make_err("require: not found");
...
lsthunk_t* lsbuiltin_prelude_include(lssize_t argc, lsthunk_t* const* args, void* data) {
  ...
  lstenv_t* tenv = (lstenv_t*)data;
  if (!tenv) {
    return ls_make_err("include: no env");
  }
  lsthunk_t* pathv = ls_eval_arg(args[0], "include: path");
  if (lsthunk_is_err(pathv))
    return pathv;
  if (!pathv) {
    return ls_make_err("include: path eval");
  }
  if (lsthunk_get_type(pathv) != LSTTYPE_STR) {
  ...
  const lsprog_t* prog = ls_require_resolve(path);
  if (!prog) {
    return ls_make_err("include: not found");
  }
    ...
    const char* rt =
        ret ? (lsthunk_is_err(ret) ? "bottom"
                                   : (lsthunk_get_type(ret) == LSTTYPE_ALGE      ? "alge"
                                      ...
                                      : lsthunk_get_type(ret) == LSTTYPE_BOTTOM  ? "bottom"
                                                                                 : "?"))
            : "NULL";
    lsprintf(stderr, 0, "DBG: include: end eval ret=%p type=%s\n", (void*)ret, rt);
  }
  if (!ret) {
    return ls_make_err("include: eval");
  }
  return ret;
}
