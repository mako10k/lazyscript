  ...
  if (exedir[0]) {
    char tmp[PATH_MAX];
    {
      ...
      if (need < 0 || (size_t)need >= sizeof(tmp))
        ...
        snprintf(tmp, (size_t)need + 1, "%s/builtins", exedir);
    }
    append_path(out, outsz, tmp, NULL);
    {
      ...
      if (need < 0 || (size_t)need >= sizeof(tmp))
        ...
        snprintf(tmp, (size_t)need + 1, "%s/plugins", exedir);
    }
    append_path(out, outsz, tmp, NULL);
    // When executed from the build tree, shared objects live in .libs/
    {
      ...
      if (need < 0 || (size_t)need >= sizeof(tmp))
        ...
        snprintf(tmp, (size_t)need + 1, "%s/builtins/.libs", exedir);
    }
    append_path(out, outsz, tmp, NULL);
    {
      ...
      if (need < 0 || (size_t)need >= sizeof(tmp))
        ...
        snprintf(tmp, (size_t)need + 1, "%s/plugins/.libs", exedir);
    }
    append_path(out, outsz, tmp, NULL);
  }
...
static void make_candidate(char* out, size_t outsz, const char* dir, const char* name,
                           int variant) {
  // variant 0: <dir>/liblazyscript_<name>.so
  // variant 1: <dir>/<name>.so
  int needed = (variant == 0) ? snprintf(NULL, 0, "%s/liblazyscript_%s.so", dir, name)
...
static lsthunk_t* make_symbol_key(const char* name) {
  ...
  if (n + 2 >= sizeof(buf))
    n = sizeof(buf) - 2; // truncate long names
  buf[0] = '.';
  memcpy(buf + 1, name, n);
  buf[1 + n] = '\0';
...
static lsthunk_t* build_namespace_from_module(const ls_builtin_module_t* mod) {
  if (!mod || !mod->entries || mod->entry_count < 0)
    return ls_make_err("builtin: invalid module");
...
lsthunk_t* lsbuiltin_prelude_builtin(lssize_t argc, lsthunk_t* const* args, void* data) {
  (void)data;
  if (argc != 1)
    return ls_make_err("builtin: arity");
  lsthunk_t* namev = ls_eval_arg(args[0], "builtin: name");
  if (lsthunk_is_err(namev))
    return namev;
  if (!namev)
    return ls_make_err("builtin: name eval");
  if (lsthunk_get_type(namev) != LSTTYPE_STR)
    return ls_make_err("builtin: expected string");
  const lsstr_t* s       = lsthunk_get_str(namev);
  const char*    modname = lsstr_get_buf(s);
  if (!modname || !*modname)
    return ls_make_err("builtin: empty name");

  // Cache lookup
  if (!g_builtin_cache)
    g_builtin_cache = lshash_new(16);
  lshash_data_t cached;
  if (lshash_get(g_builtin_cache, s, &cached)) {
    if (debug_enabled())
      lsprintf(stderr, 0, "I: builtin: cache hit: name=%s\n", modname);
  ...
  if (modname[0] == '/' || strstr(modname, "/") || strstr(modname, ".so")) {
    strncpy(chosen, modname, sizeof(chosen) - 1);
    chosen[sizeof(chosen) - 1] = '\0';
  } else {
    // Build search list
    char paths[4096];
    build_search_paths(paths, sizeof(paths));
    const char* p = paths;
    while (p && *p) {
      ...
      if (file_exists(cand)) {
        strncpy(chosen, cand, sizeof(chosen) - 1);
        chosen[sizeof(chosen) - 1] = '\0';
        break;
      }
  ...
  if (!chosen[0]) {
    if (debug_enabled())
      lsprintf(stderr, 0, "W: builtin: not found: name=%s\n", modname);
  ...
  if (!handle) {
    ...
    (void)err;
    if (debug_enabled())
      lsprintf(stderr, 0, "W: builtin: dlopen failed: path=%s err=%s\n", chosen,
               err ? err : "(null)");
  ...
  if (derr != NULL || !initfn) {
    ...
    if (!initfn) {
      if (debug_enabled())
        lsprintf(stderr, 0, "W: builtin: missing init symbol in %s\n", chosen);
      return ls_make_err("builtin: missing init symbol");
    }
  }
  const ls_builtin_module_t* mod = initfn();
  if (!mod)
    return ls_make_err("builtin: init failed");
  if (mod->abi_version != 1)
    return ls_make_err("builtin: abi version");

  lsthunk_t* ns = build_namespace_from_module(mod);
  if (lsthunk_is_err(ns) || !ns)
    return ns ? ns : ls_make_err("builtin: ns build");
  ...
  if (debug_enabled())
    lsprintf(stderr, 0, "I: builtin: loaded: name=%s path=%s entries=%d\n", modname, chosen,
             mod->entry_count);
  return ns;
}
