...
void lsprog_print(FILE* fp, int prec, int indent, const lsprog_t* prog) {
  ...
  if (lsfmt_is_active()) {
    int first_line = 0;
    if (prog && prog->lp_expr) first_line = lsexpr_get_loc(prog->lp_expr).first_line;
...
void lsfmt_flush_comments_up_to(FILE* fp, int line, int indent) {
  ...
  while (g_fmt_index < n) {
    ...
    if (!c || c->lc_loc.first_line > line) break;
  if (g_fmt_hold_line && c->lc_loc.first_line == g_fmt_hold_line) break; // keep EOL for caller
    if (c->lc_text) {
...
const lscomment_t* lsfmt_peek_next_comment(void) {
  if (!g_fmt_comments) return NULL;
  const void* const* pv = lsarray_get(g_fmt_comments);
  lssize_t n = lsarray_get_size(g_fmt_comments);
  if (g_fmt_index >= n) return NULL;
