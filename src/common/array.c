...
const void* const* lsa_concatv(lssize_t size1, const void* const* ary1, lssize_t size2,
                               va_list ap) {
  if (size1 == 0)
...
const void* const* lsa_splicev(lssize_t size, const void* const* ary, lssize_t start, lssize_t end,
  ...
  const void** new_ary = lsmalloc((size - end + start + insize) * sizeof(void*));
...
const lsarray_t* lsarray_concat(const lsarray_t* ary1, const lsarray_t* ary2) {
  ...
  if (ary2 == NULL)
    return ary1;
  lsarray_t* ary = lsmalloc(sizeof(lsarray_t));
  ary->la_values = lsa_concata(ary1->la_size, ary1->la_values, ary2->la_size, ary2->la_values);
...
const lsarray_t* lsarray_splicev(const lsarray_t* ary, lssize_t start, lssize_t end,
  ...
  if (start == end && insize == 0)
    return ary;
  lsarray_t* new_ary = lsmalloc(sizeof(lsarray_t));
  new_ary->la_values = lsa_splicev(ary->la_size, ary->la_values, start, end, insize, ap);
...
const lsarray_t* lsarray_splicea(const lsarray_t* ary, lssize_t start, lssize_t end,
  ...
  if (start == end && lsarray_get_size(ins) == 0)
    return ary;
  lsarray_t* new_ary = lsmalloc(sizeof(lsarray_t));
  new_ary->la_values = lsa_splicea(ary->la_size, ary->la_values, start, end, lsarray_get_size(ins),
                                   lsarray_get(ins));
  new_ary->la_size   = ary->la_size - end + start + lsarray_get_size(ins);
  return new_ary;
}

const lsarray_t* lsarray_splice(const lsarray_t* ary, lssize_t start, lssize_t end, lssize_t insize,
                                ...) {
  va_list ap;
  va_start(ap, insize);
