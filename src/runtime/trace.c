...
static void free_table(lstrace_table_t* t) {
  ...
  if (t->spans) {
    ...
    for (int i = 0; i < t->count; i++) {
      ...
      if (t->spans[i].filename)
        free((void*)t->spans[i].filename);
    }
    free(t->spans);
  }
  free(t);
...
void lstrace_free(void) {
  ...
  g_trace_top     = 0;
  if (g_trace_dump_fp) {
    fclose(g_trace_dump_fp);
    g_trace_dump_fp = NULL;
  }
}
...
int lstrace_load_jsonl(const char* path) {
  ...
  while (getline(&line, &n, fp) != -1) {
    ...
    if (q)
      q = strchr(q + 1, '"'); // second quote after key
...
lstrace_span_t lstrace_lookup(int index) {
  ...
  if (!g_lstrace_table)
    return d;
  if (index < 0 || index >= g_lstrace_table->count)
    return d;
...
void lstrace_emit_loc(lsloc_t loc) {
  ...
  fprintf(g_trace_dump_fp, ",\"sl\":%d,\"sc\":%d,\"el\":%d,\"ec\":%d}", loc.first_line,
...
lsloc_t lstrace_take_pending_or_unknown(void) {
  if (g_has_pending_loc) {
    g_has_pending_loc = 0;
    return g_pending_loc;
  }
