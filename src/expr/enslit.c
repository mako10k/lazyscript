...
void lsenslit_print(FILE* fp, lsprec_t prec, int indent, const lsenslit_t* ns, lsloc_t loc) {
  ...
  if (lsfmt_is_active() && n > 0) {
    ...
    for (lssize_t i = 0; i < n; i++) {
      ...
      if (ln > 0 && (min_line == 0 || ln < min_line)) min_line = ln;
    }
    if (min_line > 0) {
  ...
  for (lssize_t i = 0; i < n; i++) {
    ...
    if (lsfmt_is_active()) {
      ...
      if (rl > 0) lsfmt_flush_comments_up_to(fp, rl - 1, indent + 1);
    }
  ...
  if (lsfmt_is_active()) lsfmt_set_hold_line(lsexpr_get_loc(body).first_line);
  lsexpr_print(fp, LSPREC_LOWEST, indent + 1, body);
  if (lsfmt_is_active()) lsfmt_clear_hold_line();
    } else {
      lsprintf(fp, 0, " = ");
      // Print expression; allow nested printers to indent relative to current level
  if (lsfmt_is_active()) lsfmt_set_hold_line(lsexpr_get_loc(rhs).first_line);
  ...
  if (lsfmt_is_active()) lsfmt_clear_hold_line();
    ...
    if (lsfmt_is_active()) {
      ...
      int rhs_line = lsexpr_get_loc((lsexpr_typeof(rhs) == LSEQ_LAMBDA) ? lselambda_get_body(lsexpr_get_lambda(rhs)) : rhs).first_line;
        ...
        if (s && s[0] == '#') {
          ...
          if (i + 1 == n) {
            ...
            if (lsfmt_is_active()) {
              ...
              while (nc2 && nc2->lc_loc.first_line <= (loc.last_line > 0 ? loc.last_line - 1 : INT_MAX)) {
                if (nc2->lc_text) {
                  const char* s2 = lsstr_get_buf(nc2->lc_text);
                  if (s2 && s2[0]) lsprintf(fp, indent + 1, "%s\n", s2);
                }
                lsfmt_consume_next_comment();
