...
static const lscir_expr_t *lower_expr(const lsexpr_t *e) {
  ...
  // NOTE (policy): No name-based special-casing (e.g., prelude/println/exit) here.
  // If effectful ops or imports are needed, they must be represented explicitly in IR
  // via a future import/registry. This lowerer only performs generic application lowering.
  ...
  const char* opnm = (lsechoice_get_kind(ch) == LSECHOICE_LAMBDA) ? "choice.lambda" :
                     (lsechoice_get_kind(ch) == LSECHOICE_EXPR) ? "choice.expr" :
                                                                  "choice.catch";
  const lscir_value_t *f = mk_val_var(opnm); const lscir_value_t **args_arr = lsmalloc(sizeof(const lscir_value_t*) * 2); args_arr[0] = vl; args_arr[1] = vr; const lscir_expr_t *core = mk_exp_app(f, 2, args_arr); if (rn) core = mk_exp_let(rn, lower_expr(r), core); if (ln) core = mk_exp_let(ln, lower_expr(l), core); return core;
  }
...
static void cir_print_pat(FILE *ofp, int ind, const lscir_pat_t *p);
...
static void cir_print_val(FILE *ofp, int ind, const lscir_value_t *v) {
  ...
  case LCIR_VAL_NSLIT: lsprintf(ofp, ind, "(nslit ...)"); return;
  }
}
...
static void cir_print_expr(FILE *ofp, int ind, const lscir_expr_t *e2) {
  ...
  case LCIR_EXP_MATCH: {
    lsprintf(ofp, ind, "(match ");
    cir_print_val(ofp, 0, e2->match1.scrut);
    for (int i = 0; i < e2->match1.casec; i++) {
      fputc(' ', ofp);
      fputc('(', ofp);
  const lscir_case_t *c = &e2->match1.cases[i];
  cir_print_pat(ofp, 0, c->pat);
  fprintf(ofp, " -> ");
  cir_print_expr(ofp, 0, c->body);
      fputc(')', ofp);
    }
    fputc(')', ofp);
    return;
...
static void cir_print_pat(FILE *ofp, int ind, const lscir_pat_t *p) {
  if (!p) { fprintf(ofp, "_"); return; }
  switch (p->kind) {
    case LCIR_PAT_WILDCARD:
      fprintf(ofp, "_"); return;
    case LCIR_PAT_VAR:
      fprintf(ofp, "%s", p->var ? p->var : "_"); return;
    case LCIR_PAT_INT:
      fprintf(ofp, "%lld", p->ival); return;
    case LCIR_PAT_STR:
      fprintf(ofp, "\"%s\"", p->sval ? p->sval : ""); return;
    case LCIR_PAT_CONSTR: {
      lsprintf(ofp, ind, "%s", p->constr.name ? p->constr.name : "<constr>");
      if (p->constr.subc > 0) {
        fputc('(', ofp);
        for (int i = 0; i < p->constr.subc; i++) {
          if (i) fputc(' ', ofp);
          cir_print_pat(ofp, 0, p->constr.subpats[i]);
        }
        fputc(')', ofp);
      }
      return;
...
static void cir_val_repr(char *buf, size_t n, const lscir_value_t *v) {
  ...
  switch (v->kind) {
    ...
    case LCIR_VAL_NSLIT: snprintf(buf, n, "nslit"); return;
  }
  snprintf(buf, n, "<val>");
}
...
//  - Do not use fixed identifiers in implementation logic.
// TODO: Import-based builtin/effect resolution for the clean evaluator is not implemented yet.
//       Unknown variables remain symbols; effects must be called via explicit imports later.
static const rv_t *eval_value(FILE *outfp, const lscir_value_t *v, eval_ctx_t *ctx) {
  (void)outfp; switch (v->kind) {
  case LCIR_VAL_INT: return rv_int(v->ival);
  case LCIR_VAL_STR: return rv_str(v->sval);
  case LCIR_VAL_VAR: { const rv_t *found = env_lookup(ctx ? ctx->env : NULL, v->var); if (found) return found; return rv_sym(v->var); }
  ...
  case LCIR_VAL_NSLIT: {
    // TODO: NS literal handling will be delegated to prelude or an explicit IR construct later.
    return rv_unit();
  }
  } return rv_unit();
}
...
static const rv_t *apply_natfun(FILE *outfp, const rv_t *nf, int argc, const rv_t *const *args, eval_ctx_t *ctx) {
  ...
  // TODO: Apply native functions via explicit registry (from imports). No name-based behavior here.
  (void)outfp; (void)args; return rv_unit();
}
...
// Pattern matching helper: build a new environment when pattern matches; returns 1 on success, 0 on failure.
static int pat_match_env(const lscir_pat_t *p, const rv_t *v, env_bind_t *base, env_bind_t **out_env) {
  if (!p) { *out_env = base; return 1; }
  switch (p->kind) {
    case LCIR_PAT_WILDCARD:
      *out_env = base; return 1;
    case LCIR_PAT_VAR:
      *out_env = env_push(base, p->var ? p->var : "_", v); return 1;
    case LCIR_PAT_INT:
      if (v && v->kind == RV_INT && v->ival == p->ival) { *out_env = base; return 1; }
      return 0;
    case LCIR_PAT_STR:
      if (v && v->kind == RV_STR && v->sval && p->sval && strcmp(v->sval, p->sval) == 0) { *out_env = base; return 1; }
      return 0;
    case LCIR_PAT_CONSTR: {
      if (!v || v->kind != RV_CONSTR) return 0;
      if (!p->constr.name || strcmp(v->con.name, p->constr.name) != 0) return 0;
      if (p->constr.subc != v->con.argc) return 0;
      env_bind_t *acc = base;
      for (int i = 0; i < p->constr.subc; i++) {
        env_bind_t *acc2 = acc;
        if (!pat_match_env(p->constr.subpats[i], v->con.args[i], acc, &acc2)) return 0;
        acc = acc2;
      }
      *out_env = acc; return 1;
    }
  }
  return 0;
}
...
static const rv_t *eval_expr(FILE *outfp, const lscir_expr_t *e, eval_ctx_t *ctx) {
  ...
  case LCIR_EXP_EFFAPP: { const rv_t *f = eval_value(outfp, e->effapp.func, ctx); const rv_t **xs = NULL; if (e->effapp.argc > 0) { xs = lsmalloc(sizeof(rv_t*) * e->effapp.argc); for (int i = 0; i < e->effapp.argc; i++) xs[i] = eval_value(outfp, e->effapp.args[i], ctx); }
    // TODO: Effectful calls must be provided by explicit imports. No implicit println/exit handling.
    (void)f; (void)xs; return rv_unit(); }
  case LCIR_EXP_TOKEN: return rv_tok();
  case LCIR_EXP_MATCH: {
    const rv_t *sv = eval_value(outfp, e->match1.scrut, ctx);
    for (int i = 0; i < e->match1.casec; i++) {
      const lscir_case_t *c = &e->match1.cases[i];
      env_bind_t *env2 = NULL;
      if (pat_match_env(c->pat, sv, ctx->env, &env2)) {
...
static int expr_has_effects(const lscir_expr_t *e, int in_token_scope) {
  ...
  case LCIR_EXP_EFFAPP: return 1; case LCIR_EXP_TOKEN: return 0; case LCIR_EXP_MATCH: { int any = 0; for (int i = 0; i < e->match1.casec; i++) any |= expr_has_effects(e->match1.cases[i].body, in_token_scope); return any; } }
  return 0;
}

// TODO: Native function arity must come from an explicit import/registry table in CIR.
static int natfun_arity(const char *name) { (void)name; return -1; }
...
static int expr_check_arity(const lscir_expr_t *e) {
  ...
  switch (e->kind) {
    ...
    case LCIR_EXP_MATCH: {
      int errs = 0;
      for (int i = 0; i < e->match1.casec; i++) errs += expr_check_arity(e->match1.cases[i].body);
      return errs;
    }
  }
  return 0;
