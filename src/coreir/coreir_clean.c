...
static void cir_print_pat(FILE* ofp, int ind, const lscir_pat_t* p) {
  ...
  case LCIR_PAT_WILDCARD:
    ...
    return;
  case LCIR_PAT_VAR:
    fprintf(ofp, "%s", p->var ? p->var : "_");
    return;
  case LCIR_PAT_INT:
    fprintf(ofp, "%lld", p->ival);
    return;
  case LCIR_PAT_STR:
    fprintf(ofp, "\"%s\"", p->sval ? p->sval : "");
    return;
...
// Pattern matching helper: build a new environment when pattern matches; returns 1 on success, 0 on
// failure.
static int pat_match_env(const lscir_pat_t* p, const rv_t* v, env_bind_t* base,
                         env_bind_t** out_env) {
  if (!p) {
    *out_env = base;
    return 1;
  ...
  case LCIR_PAT_WILDCARD:
    *out_env = base;
    return 1;
  case LCIR_PAT_VAR:
    *out_env = env_push(base, p->var ? p->var : "_", v);
    return 1;
  case LCIR_PAT_INT:
    if (v && v->kind == RV_INT && v->ival == p->ival) {
      *out_env = base;
      return 1;
    }
    return 0;
  case LCIR_PAT_STR:
    if (v && v->kind == RV_STR && v->sval && p->sval && strcmp(v->sval, p->sval) == 0) {
      *out_env = base;
      return 1;
    }
    return 0;
  case LCIR_PAT_CONSTR: {
    if (!v || v->kind != RV_CONSTR)
      return 0;
    if (!p->constr.name || strcmp(v->con.name, p->constr.name) != 0)
      return 0;
    if (p->constr.subc != v->con.argc)
      return 0;
    env_bind_t* acc = base;
    for (int i = 0; i < p->constr.subc; i++) {
      env_bind_t* acc2 = acc;
      if (!pat_match_env(p->constr.subpats[i], v->con.args[i], acc, &acc2))
        return 0;
      acc = acc2;
    }
    *out_env = acc;
...
static const rv_t* eval_expr(FILE* outfp, const lscir_expr_t* e, eval_ctx_t* ctx) {
  ...
  case LCIR_EXP_EFFAPP: {
    ...
    // TODO: Effectful calls must be provided by explicit imports. No implicit println/exit
    // handling.
    (void)f;
    (void)xs;
    return rv_unit();
  ...
  case LCIR_EXP_MATCH: {
    ...
    // No case matched; emit a Bottom-style diagnostic and return unit (clean evaluator has no
    // thunk)
...
static int expr_has_effects(const lscir_expr_t* e, int in_token_scope) {
  ...
  case LCIR_EXP_APP: {
    int any = 0;
    for (int i = 0; i < e->app.argc; i++)
      any |= expr_has_effects(mk_exp_val(e->app.args[i]), in_token_scope);
    return any;
  }
  case LCIR_EXP_IF:
