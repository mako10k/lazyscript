...
static int cir_validate_expr(FILE *efp, const lscir_expr_t *e, eff_scope_t sc) {
  ...
  switch (e->kind) {
    ...
    case LCIR_EXP_LET: {
      ...
      eff_scope_t sc2 = sc;
      if (e->let1.bind && e->let1.bind->kind == LCIR_EXP_TOKEN) sc2.has_token = 1;
...
static int expr_has_effects(const lscir_expr_t *e, int in_token_scope) {
  ...
  switch (e->kind) {
    ...
    case LCIR_EXP_LET: {
      ...
      if (e->let1.bind && e->let1.bind->kind == LCIR_EXP_TOKEN) t = 1;
      return expr_has_effects(e->let1.bind, in_token_scope) || expr_has_effects(e->let1.body, t);
    }
    case LCIR_EXP_APP: {
      int any = 0;
      for (int i = 0; i < e->app.argc; i++) any |= val_has_effects(e->app.args[i]);
      return any;
    }
    case LCIR_EXP_IF:
      return expr_has_effects(e->ife.then_e, in_token_scope) || expr_has_effects(e->ife.else_e, in_token_scope);
    case LCIR_EXP_EFFAPP:
      return 1;
    case LCIR_EXP_TOKEN:
      return 0;
...
static int val_has_effects(const lscir_value_t *v) {
  if (v && v->kind == LCIR_VAL_LAM) return expr_has_effects(v->lam.body, 0);
  return 0;
}
